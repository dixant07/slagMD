...
filename = 'dump.random_full.lammpstrj'; % Replace with your actual file name
L = 17.750683960000;                     % Box size in Angstroms
rmax = 8.87534198;                  % Maximum radius (L/2)
dr = 0.1;                     % Bin size in Angstroms

% 1. Target Atom Types (Based on: 1:Ca, 2:Si, 3:Mg, 4:Al, 5:O)
typeA = 1; % Central atom (e.g., Silicon)
typeB = 2; % Target atom (e.g., Oxygen)
...
% =========================================================================
% 4. Normalization for Partial RDF
% =========================================================================
avg_Na = avg_Na / processed_frames;
avg_Nb = avg_Nb / processed_frames;

% Density of the TARGET atom type in the simulation box
rho_B = avg_Nb / (L^3); 
...
for k = 1:nbins
    ...
    % g_AB(r) = count / (frames * N_A * ideal_density_of_B_in_shell)
    g_AB(k) = g_hist(k) / (processed_frames * avg_Na * n_ideal_B);
end
...
filename = "rdf" + typeA + ".png";
saveas(fig, filename);

% close(fig);


% r_axis and g_AB are column or row vectors of same length
% Example: r_axis = linspace(0,10,100); g_AB = sin(r_axis).*exp(-0.1*r_axis);

% 1) global maximum
[gmax, idx_max] = max(g_AB);     % gmax is value, idx_max index of first maximum
r_at_max = r_axis(idx_max);


% 2) first local minimum (by r increasing)
minMask = islocalmin(g_AB);      % logical vector of local minima
idx_min = find(minMask, 1, 'first');  % first local minimum index
if isempty(idx_min)
    r_first_min = [];            % no local minimum found
else
